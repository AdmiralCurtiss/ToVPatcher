    ...
    public partial class Form1 : Form
    {
        ...
        public void UpdateSingleCharacter()
        {
            try
            {
				...
				int scale = 8;
				int charlen = FontInfos[CurrentFontInfoToUse].CharacterLengths[CharacterID];
                    ...
                    for (int y = 0; y < 32; y++)
                    {
						...
						for ( int xx = 0; xx < scale; ++xx ) {
							for ( int yy = 0; yy < scale; ++yy ) {
								img.SetPixel( x*scale + xx, y*scale + yy, px );
							}
						}
                    }
                }
				...
				try {
					...
					for ( int y = 0; y < 32 * scale; ++y ) {
						img.SetPixel( charlen * scale, y, Color.LightBlue );
					}
					for ( int x = 0; x < 32 * scale; ++x ) {
						img.SetPixel( x, 30 * scale, Color.LightBlue );
					}
				} catch ( Exception ) { }
		...
		private void TownMapIDBox_KeyDown( object sender, KeyEventArgs e ) {
			try {
				if ( e.KeyCode == Keys.Left ) {
					numUpDownCharLength.Value -= 1;
				} else if ( e.KeyCode == Keys.Right ) {
					numUpDownCharLength.Value += 1;
				}
			} catch ( Exception ) {
				numUpDownCharLength.Value = 16;
			}
		}
    }
}
