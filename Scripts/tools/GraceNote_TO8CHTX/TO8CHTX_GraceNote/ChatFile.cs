...
namespace TO8CHTX_GraceNote {
	...
	class ChatFile {
		...
		public void GetSQL( String ConnectionString, bool updateGracesEnglish = false ) {
			...
			using ( SQLiteCommand Command = new SQLiteCommand( Connection ) ) {
				Command.CommandText = "SELECT english, PointerRef, StringID FROM Text ORDER BY PointerRef";
				...
				while ( r.Read() ) {
					...
					if ( updateGracesEnglish ) {
						if ( PointerRef % 16 == 0 ) {
							int index = ( PointerRef / 16 ) - 2;
							UpdateGracesJapanese( transactionGracesEnglish, StringID, Lines[index].SName, 0 );
						} else if ( PointerRef % 16 == 4 ) {
							int index = ( ( PointerRef - 4 ) / 16 ) - 2;
							UpdateGracesJapanese( transactionGracesEnglish, StringID, Lines[index].SENG, 0 );
						}
					}
