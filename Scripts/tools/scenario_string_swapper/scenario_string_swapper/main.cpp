...
int main(int argc, char ** argv) {
	...
	*/
	...
	outfile.write( buffer, textstart );

	char * charrayLocation = new char[100];
	char * charrayPointer = new char[100];
	unsigned int lastpointer = 0;

	for ( int cur = textstart; cur < filesize; cur += 4 ) {
		unsigned int * ptr1p = (unsigned int *)&buffer[cur];
		unsigned int ptr1 = *ptr1p;
		swap_endian( ptr1 );
		unsigned int * ptr2p = (unsigned int *)&buffer[cur+4];
		unsigned int ptr2 = *ptr2p;
		swap_endian( ptr2 );
		unsigned int * ptr3p = (unsigned int *)&buffer[cur+8];
		unsigned int ptr3 = *ptr3p;
		swap_endian( ptr3 );
		unsigned int * ptr4p = (unsigned int *)&buffer[cur+12];
		...
		*/
		if (   ptr1 != 0 && ptr1 < textsize && ptr1 < cur-textstart
			...
			// swap pointer1 with pointer3, pointer2 with pointer4
				...
				sprintf(charrayLocation, "%x", cur);
				sprintf(charrayPointer, "%x", ptr1+textstart);
				textdumpfile.write( "JPN NAME @ 0x", 13);
				...
				textdumpfile.write( "JPN TEXT @ 0x", 13);
				textdumpfile.write( charrayPointer, getbytestillnull(charrayPointer) );
				textdumpfile.write( ":", 1);
				...
				textdumpfile.write( "ENG NAME @ 0x", 13);
				textdumpfile.write( charrayPointer, getbytestillnull(charrayPointer) );
				textdumpfile.write( ":", 1);
				...
				textdumpfile.write( "ENG TEXT @ 0x", 13);
				textdumpfile.write( charrayPointer, getbytestillnull(charrayPointer) );
				textdumpfile.write( ":", 1);
